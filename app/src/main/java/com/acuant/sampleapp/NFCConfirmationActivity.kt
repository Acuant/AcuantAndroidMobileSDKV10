...
import com.acuant.acuantmobilesdk.models.NFCTagReadingListener
...
class NFCConfirmationActivity : Activity(), NFCTagReadingListener {
    ...
    fun nfcPressed(v: View) {
        if (nfcAdapter != null) {
            ensureSensorIsOn()
            val controller = AppInstance.instance.getController()
            if (controller != null) {
                controller.listenNFC(this, nfcAdapter, this)
