package com.acuant.sampleapp.facialcamera
...
import com.acuant.acuantmobilesdk.LiveFaceListener
import com.acuant.acuantmobilesdk.internal.livefaceprocessing.LiveFaceDetector
import com.acuant.acuantmobilesdk.models.LiveFaceDetails
...
class FacialLivelinessActivity : AppCompatActivity(), LiveFaceListener {
    ...
    private fun createCameraSource() {
        ...
        liveFaceDetector = AppInstance.instance.getController()!!.createLiveFaceDetector(context, this)
        ...
        if (liveFaceDetails.error == null) {
            ...
            if(liveFaceDetails.isLiveFace){
                if(selfieCaptured==false) {
                    selfieCaptured = true
                    thread() {
                        CapturedImage.setImage(liveFaceDetails.image)
                        val result = Intent()
                        this@FacialLivelinessActivity.setResult(Constants.REQUEST_CAMERA_SELFIE, result)
                        this@FacialLivelinessActivity.finish()
                    }
                }
            }
        } else {
